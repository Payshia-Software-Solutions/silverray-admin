...
import { createBookingRequest, getRoomTypes, type RoomTypeFromApi } from '@/lib/services/api';
...
export default function NewBookingRequestPage() {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting }, control } = useForm<RequestFormValues>({
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                toast({ variant: 'destructive', title: 'Error fetching data', description: 'Could not load room types.' });
            }
        }
        fetchData();
    }, [toast]);
    ...
    const onSubmit: SubmitHandler<RequestFormValues> = async (data) => {
        ...
        try {
            await createBookingRequest(dataToSend as any);
            toast({
                title: 'Success!',
                description: 'New booking request created successfully.',
            });
    ...
    return (
        <>
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
                <Card>
                    <CardHeader>
                        <CardTitle>Create New Booking Request</CardTitle>
                    </CardHeader>
                    <CardContent>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Controller name="booking_status" control={control} render={({ field }) => (
                                        <Select onValueChange={field.onChange} defaultValue={field.value}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Pending">Pending</SelectItem><SelectItem value="Confirmed">Confirmed</SelectItem><SelectItem value="Cancelled">Cancelled</SelectItem></SelectContent></Select>
                                    )} />
